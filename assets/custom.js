...
function updateFreeGiftProgress(cart) {
  ...
  const beforeText = wrapper.dataset.beforeText;
  const afterText = wrapper.dataset.afterText;
  ...
  if (total < goal) {
    ...
    notice.innerHTML = beforeText.replace('{amount}', `$${remaining}`);
  } else {
    notice.innerHTML = afterText;
  }
}
